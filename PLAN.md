...
### Фаза 3: Зарплаты Кафе + Owner View — ГОТОВО ✅

<details>
<summary>Детали реализации (свернуто)</summary>

**Зарплаты Кафе (страница Админа):**
- 3 роли: Кассир, Сушист, Повар Сандей — ввод имени + суммы (без автоматического расчёта)
- Админ вводит зарплаты первым делом → затем переходит к закрытию смены
- API endpoints: `GET /api/cafe/salaries/status`, `GET /api/cafe/employees/last`, `POST /api/cafe/salaries/create`
- Категории: Кассир=16, Сушист=17, Повар Сандей=auto-detect из API
- Счёт: Оставил в кассе (ID=5)
- Защита от дублей: `salaries_created` + `salaries_data` в `shift_closings`
- Автоподстановка имён и сумм из прошлого раза

**Owner View:**
- Owner свободно перемещается между страницами Кассира и Админа
- На странице Кассира: все 4 шага видны одновременно, readonly, без кнопок действий
- На странице Кафе: обе секции (зарплаты + закрытие смены) видны одновременно
- `user_role` передаётся в шаблоны, `IS_OWNER` определяет поведение в JS
- Если данные уже заполнены — показываются readonly, иначе помечаются "(не заполнено)"

</details>

### Фаза 3.5: Оптимизация ежедневных транзакций — ГОТОВО ✅

<details>
<summary>Детали реализации (свернуто)</summary>

**Убраны автоматические переводы в 12:00 (дублировали закрытие смены):**
- ❌ Pizzburg: Инкассация→Оставил в кассе (1₸), Каспий→Вольт (1₸), Каспий→Халык (1₸)
- ❌ Pizzburg-cafe: Инкассация→Оставил в кассе (1₸), Каспий→Вольт (1₸)
- ✅ Оставлен: Оставил в кассе → Деньги дома ("Забрал - Имя")

**Добавлены новые транзакции:**
- ✅ Pizzburg-cafe: Повар Сандей (1₸, auto-detect категории)
- ✅ Pizzburg: Зарплаты (1₸, auto-detect категории, комментарий "Мадина админ")

**Убраны устаревшие напоминания:**
- ❌ 21:30 — Напоминание о зарплатах (теперь кассир делает через сайт)
- ❌ 22:30 — Напоминание о сверке счетов (теперь через сайт расходов)
...
### Rate limit на /login
- Flask-Limiter для защиты от перебора паролей
- Некритично (внутренний инструмент), но хорошая практика

### Статистика / Dashboard
- Сейчас endpoint `/api/statistics` возвращает mock-данные
- Графики выручки, расходов, зарплат за период
