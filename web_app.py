        ...
        if sd_with_items:
            if 'items' in sd_with_items:
                for item in sd_with_items['items']:
                    ...
                    if 'sum' not in item:
                        item['sum'] = item.get('total') or (item.get('quantity', 0) * item.get('price', 0))
...
def ensure_purchase_templates_seeded(db, telegram_user_id: int):
    """Check if there are already suppliers for this user. If not, auto-seed defaults."""
    ...
    logger.info(f"Seeding default purchase template for user {telegram_user_id}")
    template = {
        "Инарин (Аст)": (
            {"6": 4, "3": 4},
            [
        ...
        "Янс (Аст)": (
            ...
            [
                ...
                ("Фри", 500.0),
                ("Уголок", 500.0),
                ("Упаковка Донер", 500.0),
                ("Стакан 0.3л", 1000.0),
                ("Пакет 250*140", 500.0),
                ("Соус тары", 8.0),
                ("Салфетки", 75.0),
                ("Лента 80мм", 2.0),
                ("Тарелки/Трубки/Вилки", 1.0),
                ("Перчт повар", 3.0),
                ("Перчт тонк", 8.0),
                ("Крыло 24", 1.0),
                ("Крыло 12 упк", 1.0),
                ("Крыло 6шт (зал/дост)", 1.0),
                ("Хот-Дог (зал/дост)", 1.0),
                ("Шпажки", 5.0),
                ("Мусор мешок 120/240 л", 1.0),
                ("Бахилы", 1.0)
            ]
        ),
        "Сказка": (
    ...
    for supplier_name, (schedule, ingredients) in template.items():
        supplier_id = db.add_purchase_supplier(telegram_user_id, supplier_name, schedule)
        if supplier_id != -1:
            for idx, (ing_name, target) in enumerate(ingredients):
                db.add_purchase_ingredient(
                    telegram_user_id=telegram_user_id,
                    supplier_id=supplier_id,
                    name=ing_name,
                    default_target_stock=target,
                    sort_order=idx * 10
                )
...
def api_purchase_blank():
    ...
    # Auto-seed default template if first time
    ensure_purchase_templates_seeded(db, telegram_user_id)
...
def _find_prepared_expense_for_supply(
    ...
    The expense entered on the site is preferred and is the source of truth for
    the account and amount. The returned status distinguishes a genuinely
    missing expense (a new one may be created) from an ambiguous match.
    """
    from matchers import normalize_supplier_text
    from rapidfuzz import fuzz
            ...
            # 4. Create Supply
                ...
                if is_webhook:
                    if not items:
                        ...
                        continue
                    prepared_expense, prepared_supply, match_status = _find_prepared_expense_for_supply(
                        db=db,
                        user_id=user_id,
                    ...
                    if match_status == 'ambiguous':
                        ...
                        response_text = (
                            (response_text + "\n" if response_text else "")
                            + f"⚠️ Для накладной «{display_supplier}» на {invoice_total:,.0f}₸ "
                              "нашлось несколько похожих расходов. Новый дубликат не создаю — "
                              "уточните нужный расход на сайте."
                        )
                        logger.warning(
                            "Webhook supply match is ambiguous: supplier=%s invoice_total=%s items_total=%s date=%s",
                            display_supplier,
                            invoice_total,
                            items_total,
                            date_str,
