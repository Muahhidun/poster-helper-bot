...
"""
        ...
        parsing_prompt = f"""
Ты система обработки накладных. Вот ТОЧНЫЙ текст с накладной (распознан через OCR):

---
{ocr_text}
---

Твоя задача - извлечь из этого текста:

1. Название поставщика (ищи ТОО, ИП, ООО в начале документа)
2. Дата накладной (преобразуй в формат YYYY-MM-DD)
3. ВСЕ товары из таблицы - для КАЖДОЙ строки:
   - Название товара (полное, со всеми характеристиками)
   - Количество (из колонки "подлежит отпуску" или "количество")
   - Единица измерения (упак, шт, кг, л)
   - Цена за единицу

КРИТИЧЕСКИ ВАЖНО:
✅ Извлеки ВСЕ позиции товаров (не пропускай ни одной!)
✅ Названия товаров - ТОЧНО как в тексте
✅ Если видишь дубли (одинаковые названия) - включи их все отдельно
❌ НЕ придумывай товары, которых нет в тексте
❌ НЕ объединяй разные товары
❌ НЕ пропускай строки таблицы

Верни ТОЛЬКО JSON (без markdown, без комментариев):
{{
    "supplier_name": "Название поставщика из текста",
