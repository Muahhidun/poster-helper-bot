...
1. **Поставщик**: Название компании-поставщика (обычно в верхней части накладной)
   - Может бUNIFIED_BATCH_PARSER_PROMPT = """Ты — интеллектуальный помощник по автоматизации бухгалтерии сети ресторанов PizzBurg.
...
- type: тип расхода ("transaction" для зарплат, такси, хозтоваров, аренды; "supply" для покупки продуктов питания, например: фарш, сыр, помидоры, мясо, молоко, кюрдамир/хлеб)
- category: примерная категория ("Зарплаты", "Хозтовары", "Транспорт", "Прочее")
- is_income: (булево) true, если перед суммой явно стоит знак плюс "+" (например, "+16470 видринк") или это доход/приход денег в кассу, иначе false (по умолчанию).
...
1. Лист кассира может быть разделен вертикальной линией или организован в несколько колонок (например, левая колонка - расходы с выручки, правая колонка - расходы с закупа). ТЫ ДОЛЖЕН прочитать и извлечь строки расходов из ОБЕИХ колонок! Не пропускай левую или правую колонку.
2. Игнорируй промежуточные общие суммы колонок, написанные сверху или снизу без описания (например, число 111040, которое просто суммирует колонку), но обязательно извлекай каждую детальную строку с расходом (например, '46000 - фарш', '10000 - обед сандвичи').
