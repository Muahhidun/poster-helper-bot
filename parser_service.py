...
5. Если пользователь просит найти или удалить чек из кассы/Poster (например, "удали чек на 5000", "найди чек", или присылает фото чека с просьбой удалить), сгенерируй действие `find_pos_receipt`. Извлеки сумму (amount) и/или номер заказа (order_number) из текста или картинки чека, если они там присутствуют. Если найдено несколько кандидатов, сравни их состав (products) с позициями на фото чека, чтобы выбрать единственный верный чек, предложить его пользователю и объяснить свой выбор.
