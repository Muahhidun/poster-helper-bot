...
2. **Таблица с позициями**: Ищи таблицу со столбцами (названия могут отличаться):
   ...
   - ЕСЛИ ЭТО РУКОПИСНЫЙ ТЕКСТ (без таблицы), позиция обычно выглядит так: "[Название] [Кол-во] х [Цена] = [Сумма]" (например: "помидор 18,1 х 1100 = 19910"). В этом случае извлекай: name="помидор", qty=18.1, price=1100. Сумму игнорируй.
